...
#### 5.5.10 Namelist 物理源项参数

Step 4 界面在「数值积分时间步长参数」下方提供 **Namelist Parameters** 分组，用于调整 ST4 方案下最常率定的 5 个物理系数。填写后点击「确认参数」，数值会写入工作目录 `namelists.nml` 中对应的 namelist 段（普通网格写根目录；嵌套网格逐层写入各 `level*/namelists.nml`），并同步保存到 `params.yml` 的 `ww3.namelist` 段。

与 `ww3_grid.nml` 中的谱离散、时间步不同，这 5 项控制**风输入、白冠耗散、底摩擦**等源项强度，直接影响有效波高 $H_s$、涌浪衰减与近岸能量水平。默认值对应 WW3 6.07 ST4（Ardhuin et al. 2010）模板；

| GUI 字段 | YAML 键 | namelist 段 | 默认值 | 含义与常见调整 |
|----------|---------|-------------|--------|----------------|
| BETAMAX | `SIN4%BETAMAX` | `&SIN4` | 1.43 | 风能输入增益（Miles 系数），**最常率定**。整体 $H_s$ 偏低可调大（如 1.52、1.55），偏高可调小（如 1.33）。 |
| SWELLF | `SIN4%SWELLF` | `&SIN4` | 0.66 | 涌浪风输入衰减系数，影响远场涌浪传播与衰减；常见范围约 0.6–0.8。 |
| SDSC2 | `SDS4%SDSC2` | `&SDS4` | -2.2e-5 | 白冠饱和耗散主系数，**建议与 BETAMAX 配对调整**，共同控制总能量平衡。 |
| SDSBR | `SDS4%SDSBR` | `&SDS4` | 0.90e-3 | 饱和耗散阈值 $B_r$，影响耗散起始与谱形。 |
| GAMMA | `SBT1%GAMMA` | `&SBT1` | -0.067 | JONSWAP 底摩擦系数；**近岸/陆架必关注**。涌浪主导可试 -0.038，风浪/混合保持 -0.067。 |
...
- 先动 **BETAMAX + SDSC2** 配对调节整体 $H_s$ 与能量平衡，不要只改其中一个。
- 近岸或浅水算例若 $H_s$ 或周期偏差明显，再单独试 **GAMMA**（底摩擦）。
- 若选用 ST2/ST3 等非 ST4 源项包，上述 `&SIN4` / `&SDS4` 块可能不被读取；此时应改对应 ST 版本的 namelist 段，或保持默认。
- 留空某字段时，该键不会覆盖模板默认值（与频谱/时间步字段行为一致）。
