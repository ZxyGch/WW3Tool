...
def parse_input_args():
    ...
    parser.add_argument(
        ...
        help='Path to config.ini or config.json (JSON fixes output to grid.msh / grid.ww3).')
...
def print_gridgen_banner_start():
    """Same banner as gridgen/python/create_grid.py (unstructured mesh via JIGSAW)."""
    sys.stdout.flush()
    print("=" * 70, flush=True)
...
def print_gridgen_banner_end():
    """Same completion banner as gridgen/python/create_grid.py."""
    print("=" * 70, flush=True)
...
#-- create a simple uniform mesh for the globe
    ...
    # MARCHE reads HFUN_* from opts.jig; these must be set before marche().
    opts.hfun_scal = "absolute"
    opts.hfun_hmax = configurations["hfun_hmax"]
    opts.hfun_hmin = min(
        configurations["hmin"], configurations["hfun_hmax"])
...
#-- and a little nonlinear smoothing
    ...
    # Corner remap averages toward coast; without a second repair, deep-ocean h(x)
    # stays too small so spac.value = min(hmax, hmat*zoom) plateaus below user hmax.
...
#-- return tria-to-tria adj. as a sparse graph
    ...
    same = np.argwhere(
        np.logical_and.reduce((diff[:, 0] == 0, diff[:, 1] == 0))
    ).ravel()

    # tris[same] and tris[same+1] share
    rows = np.concatenate((tris[same], tris[same + 1]))
    cols = np.concatenate((tris[same + 1], tris[same]))
    data = np.ones(rows.size, dtype=np.int8)

    # ith tri is adj. to tri in ith row (shape required when no shared edges)
    return csr_matrix((data, (rows, cols)), shape=(n, n), dtype=np.int8)
