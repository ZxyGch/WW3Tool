...
Reads Gmsh 2.x text grids: both ``*.msh`` (Jigsaw export) and ``*.ww3`` written by
``ocn_ww3.py`` / ``ocn_ww3_regional.py`` use the same format. Other WW3-related
files (e.g. binary mod_def) are not supported here.

Map extent defaults to the node bounding box (with a small margin). Override with
``--extent`` if needed (e.g. grids crossing the dateline).
"""
...
def read_gmsh(filename):
    ...
    with open(filename, 'r') as f:
        ...
        for _ in range(ne):
            ...
            eltype = int(line[1])
            ntags = int(line[2])
            i0 = 3 + ntags  # first node index column (Gmsh: elm type ntags tag... nodes)
            if eltype == 15:
                bndtemp.append(int(line[i0]) - 1)
            elif len(line) >= i0 + 3:
                # 3-node triangle (Gmsh type 2; WW3 *.ww3 from write_gmsh_mesh)
                ecttemp[elem_count, :] = [
                    int(line[i0]) - 1,
                    int(line[i0 + 1]) - 1,
                    int(line[i0 + 2]) - 1,
...
def extent_from_xy(xy, margin_deg=None):
    """
    Bounding box in degrees from node lon/lat, with a small outer pad for framing.
    margin_deg: if None, use max(2% of max span, 0.1°).
    """
    ...
    if margin_deg is None:
        m = max(0.02 * max(lon_s, lat_s), 0.1)
    else:
        m = float(margin_deg)
    return [
        lon_min - m,
...
def plot_eleminfo(
    ...
    lon_w = extent[1] - extent[0]
    lat_h = extent[3] - extent[2]
    if lon_w > 0 and lat_h > 0:
        ar = lon_w / lat_h
        base_h = 8.0
        figsize = [min(22.0, max(5.0, base_h * ar)), base_h]
    else:
        figsize = [18.0, 9.0]
    plots = [
    ...
    for suffix, data, label, shading, cmap in plots:
        ...
        setup_plot(ax, extent=extent)
        if suffix == 'elm':
            cf = ax.triplot(
                triang, 'k-', linewidth=0.5, transform=ccrs.PlateCarree()
            )
        else:
            cf = ax.tripcolor(
                triang,
                data,
                cmap=cmap,
                shading=shading if shading else 'flat',
                vmin=vpltmin,
                vmax=vpltmax if data is not depth else None,
        ...
        plt.savefig(f'{suffix}_{plotdescriptor}.png', dpi=150)
        plt.close()

def main():
    parser = argparse.ArgumentParser(
        description='Plot mesh from a Gmsh 2.x text file (.msh or .ww3 from ocn_ww3*).'
    )
    parser.add_argument(
        '--filename',
        type=str,
        default='grid.ww3',
        help='Path to grid.msh / grid.ww3 (default: ./grid.ww3)',
    )
    ...
    parser.add_argument(
        ...
        help='Override map window (deg). Default: from grid node lon/lat + margin.',
    )
    parser.add_argument(
        '--margin-deg',
        type=float,
        default=None,
        help='Pad around data bbox in degrees (default: max(2%% of span, 0.1°)).',
    )
    ...
    if not os.path.isfile(filename):
        print(
            ...
            f"{filename}\n"
            f"(cwd: {os.getcwd()})\n"
            "Use grid.msh or grid.ww3 from ocn_ww3.py / ocn_ww3_regional.py (same Gmsh text format).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if args.extent is not None:
        extent = [float(x) for x in args.extent]
        if extent[1] <= extent[0] or extent[3] <= extent[2]:
            print(
                "Invalid --extent: need lon_max > lon_min and lat_max > lat_min.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
        extent = extent_from_xy(xy, margin_deg=args.margin_deg)
