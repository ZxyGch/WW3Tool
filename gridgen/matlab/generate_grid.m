...
%|                     'gebco'  -- GEBCO bathymetry (gebco.nc)            |
...
%|                   For the gebco.nc file these are 'lon', 'lat' and 'elevation'|
%|                                                                        |
%| OUTPUT                                                                 |
...
if (lons >= 0 && lone > 180)
    ...
    % Convert values > 180 to negative
    x_converted = x;
    x_converted(x > 180) = x(x > 180) - 360;
    lons = min(min(x_converted));
    lone = max(max(x_converted));
    x = x_converted;
    lon_converted = true;
    fprintf(1,'  Converted longitude from 0~360 to -180~180 format: [%.2f, %.2f]\n',lons,lone);
end
...
elseif (strcmp(bathy_source,'etopo1'))
    ...
    var_y = 'lat';
    var_z = 'z';    
elseif (strcmp(bathy_source,'gebco'))
    fname_base = [ref_dir,'/gebco.nc'];
    var_x = 'lon';
    var_y = 'lat';
    var_z = 'elevation';
...
% Try to get actual_range attribute, if not available, read from data
try
    lat_range = netcdf.getAtt(f,varid_lat,'actual_range');
    lats_base = lat_range(1);
    late_base = lat_range(2);
catch
    % If actual_range not found, read first and last values
    % For evenly spaced grids, this is sufficient
    lat_first = netcdf.getVar(f,varid_lat,0,1);
    lat_last = netcdf.getVar(f,varid_lat,Ny_base-1,1);
    lats_base = min(lat_first, lat_last);
    late_base = max(lat_first, lat_last);
    lat_range = [lats_base, late_base];
end
if Ny_base > 1
...
try
    ...
    % If actual_range not found, read first and last values
    lon_first = netcdf.getVar(f,varid_lon,0,1);
    lon_last = netcdf.getVar(f,varid_lon,Nx_base-1,1);
    lons_base = min(lon_first, lon_last);
    lone_base = max(lon_first, lon_last);
    lon_range = [lons_base, lone_base];
end
if Nx_base > 1
...
if (lats < lats_base || lats > late_base || late < lats_base || late > ...
        late_base)
    fprintf(1,'ERROR : Lattitudes (%.2f,%.2f) beyond range (%.2f,%.2f) \n',lats,...
        late,lats_base,late_base);
    return;
end;
  
%@@@ Check longitude range with tolerance for floating point precision
...
if (lone > lone_base && lone <= 180.0 && lone_base > 179.0)
    lone_check = lone_base;  % Clamp to max
end
if (lons < lons_base && lons >= -180.0 && lons_base < -179.0)
    lons_check = lons_base;  % Clamp to min
end

if (lons_check < lons_base - lon_tolerance || lons_check > lone_base + lon_tolerance || ...
        lone_check < lons_base - lon_tolerance || lone_check > lone_base + lon_tolerance)
    fprintf(1,'ERROR : Longitudes (%.2f,%.2f) beyond range (%.2f,%.2f) \n',lons,...
