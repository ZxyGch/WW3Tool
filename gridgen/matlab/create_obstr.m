...
sx_before_neighbor = zeros(Ny,Nx);
sy_before_neighbor = zeros(Ny,Nx);
cells_with_sx_before = 0;
cells_with_sy_before = 0;
cells_with_nx_but_no_sx = 0;
cells_with_ny_but_no_sy = 0;
...
for indx_bnd = 1:N_bnd
    ...
    if (cell(k,j).nx ~=0)
        ...
        %@@@ Record value before neighbor check
        sx_before_neighbor(k,j) = sx(k,j);
        if (sx(k,j) > 0)
            cells_with_sx_before = cells_with_sx_before + 1;
        elseif (cell(k,j).nx > 0)
            cells_with_nx_but_no_sx = cells_with_nx_but_no_sx + 1;
        end;
    ...
    if (cell(k,j).ny~=0)
        ...
        if (sy(k,j) > 0)
            cells_with_sy_before = cells_with_sy_before + 1;
        elseif (cell(k,j).ny > 0)
            cells_with_ny_but_no_sy = cells_with_ny_but_no_sy + 1;
        end;
...
%@@@ Print statistics about obstruction values
sx_nonzero = sx(sx > 0);
sy_nonzero = sy(sy > 0);
sx_before_nonzero = sx_before_neighbor(sx_before_neighbor > 0);
sy_before_nonzero = sy_before_neighbor(sy_before_neighbor > 0);
total_wet = sum(mask(:) ~= 0);
total_cells = Nx * Ny;

%@@@ Count how many were zeroed by neighbor check
sx_zeroed_by_neighbor = sum((sx_before_neighbor(:) > 0) & (sx(:) == 0));
sy_zeroed_by_neighbor = sum((sy_before_neighbor(:) > 0) & (sy(:) == 0));

fprintf(1,'  Obstruction grid statistics:\n');
fprintf(1,'    Total cells: %d, Wet cells: %d\n', total_cells, total_wet);
fprintf(1,'    Cells with boundaries detected: %d\n', N_bnd);
fprintf(1,'    Cells with nx>0 but sx=0 (shadow/no_boundary): %d\n', cells_with_nx_but_no_sx);
fprintf(1,'    Cells with ny>0 but sy=0 (shadow/no_boundary): %d\n', cells_with_ny_but_no_sy);
fprintf(1,'    Cells with SX before neighbor check: %d\n', cells_with_sx_before);
fprintf(1,'    Cells with SY before neighbor check: %d\n', cells_with_sy_before);
if (cells_with_sx_before > 0)
    fprintf(1,'    SX zeroed by neighbor check: %d (%.1f%%)\n', sx_zeroed_by_neighbor, 100*sx_zeroed_by_neighbor/cells_with_sx_before);
else
    fprintf(1,'    SX zeroed by neighbor check: 0\n');
end;
if (cells_with_sy_before > 0)
    fprintf(1,'    SY zeroed by neighbor check: %d (%.1f%%)\n', sy_zeroed_by_neighbor, 100*sy_zeroed_by_neighbor/cells_with_sy_before);
else
    fprintf(1,'    SY zeroed by neighbor check: 0\n');
end;

if (length(sx_before_nonzero) > 0)
    fprintf(1,'  SX Obstruction (before neighbor check):\n');
    fprintf(1,'    Non-zero cells: %d\n', length(sx_before_nonzero));
    fprintf(1,'    Range: [%.4f, %.4f], Mean: %.4f\n', min(sx_before_nonzero), max(sx_before_nonzero), mean(sx_before_nonzero));
end;

if (length(sy_before_nonzero) > 0)
    fprintf(1,'  SY Obstruction (before neighbor check):\n');
    fprintf(1,'    Non-zero cells: %d\n', length(sy_before_nonzero));
    fprintf(1,'    Range: [%.4f, %.4f], Mean: %.4f\n', min(sy_before_nonzero), max(sy_before_nonzero), mean(sy_before_nonzero));
end;

if (length(sx_nonzero) > 0)
    fprintf(1,'  SX Obstruction (after neighbor check):\n');
    fprintf(1,'    Non-zero cells: %d / %d wet (%.2f%%)\n', length(sx_nonzero), total_wet, 100*length(sx_nonzero)/total_wet);
    fprintf(1,'    Range: [%.4f, %.4f], Mean: %.4f\n', min(sx_nonzero), max(sx_nonzero), mean(sx_nonzero));
    low = sum((sx_nonzero > 0) & (sx_nonzero < 0.1));
    mid = sum((sx_nonzero >= 0.1) & (sx_nonzero < 0.5));
    high = sum(sx_nonzero >= 0.5);
    fprintf(1,'    Distribution: low=%d, mid=%d, high=%d\n', low, mid, high);
else
    fprintf(1,'  Warning: No non-zero SX obstruction values after neighbor check!\n');
end;

if (length(sy_nonzero) > 0)
    fprintf(1,'  SY Obstruction (after neighbor check):\n');
    fprintf(1,'    Non-zero cells: %d / %d wet (%.2f%%)\n', length(sy_nonzero), total_wet, 100*length(sy_nonzero)/total_wet);
    fprintf(1,'    Range: [%.4f, %.4f], Mean: %.4f\n', min(sy_nonzero), max(sy_nonzero), mean(sy_nonzero));
    low = sum((sy_nonzero > 0) & (sy_nonzero < 0.1));
    mid = sum((sy_nonzero >= 0.1) & (sy_nonzero < 0.5));
    high = sum(sy_nonzero >= 0.5);
    fprintf(1,'    Distribution: low=%d, mid=%d, high=%d\n', low, mid, high);
else
    fprintf(1,'  Warning: No non-zero SY obstruction values after neighbor check!\n');
end;
