...
class PlotInterface(QWidget):
    ...
    def auto_detect_from_workdir(self, workdir: str | None) -> None:
        ...
        try:
            ...
            from workflows.infrastructure.runtime_config import get_project_root
            project_root = get_project_root()

            if not self._jason3_fields["folder"].text().strip():
                self._jason3_fields["folder"].setText(os.path.join(project_root, "jason3"))
            if not self._ndbc_fields["folder"].text().strip():
                self._ndbc_fields["folder"].setText(os.path.join(project_root, "ndbc"))
    ...
    def _build_wind_field_card(self, run_wind_field: Callable, view_subdir: str) -> None:
        ...
        self._wind_flag_options = [
            ("arrow", tr("plotting_wind_flag_arrow", "箭头")),
            ("barb", tr("plotting_wind_flag_flag", "风旗")),
            ("none", tr("plotting_wind_flag_none", "无")),
        ]
        ...
        for idx, (canonical, _label) in enumerate(self._wind_flag_options):
            if canonical == "arrow":
                self._wind_flag_combo.setCurrentIndex(idx)
                break
    ...
    def _build_spectrum_card(self, run_spectrum_all, run_spectrum_selected, run_spectrum_map) -> None:
        ...
        self._spectrum_mode.addItem(tr("plotting_mode_normalized", "最大值归一化"), userData="normalized")
        self._spectrum_mode.addItem(tr("plotting_mode_actual", "实际值"), userData="actual")
    ...
    def wind_field_params(self) -> dict:
        ...
        try:
            ...
            time_step = 24.0
        flag_type = self._wind_flag_combo.currentData()
        if flag_type is None:
            flag_type = self._wind_flag_combo.currentText()
        from workflows.application.plot_wind_field import normalize_wind_flag_type
