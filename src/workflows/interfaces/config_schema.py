...
def build_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "fields": [
            ...
            _f("boundary.mode", "enum", "外部边界谱：关闭或启用",
               options=["none", "external_spectra"], default="none",
               note="工作目录缺少本段时视为 none；根模板中的 files 不会自动启用其他算例。"),
            _f("boundary.source.format", "enum", "谱文件格式",
               options=["ww3_netcdf"], default="ww3_netcdf"),
            _f("boundary.source.location", "enum", "谱文件所在位置",
               options=["local", "remote"], default="local"),
            _f("boundary.source.files", "list[path]", "WW3 点谱 NetCDF 明确列表",
               default=[],
               note="启用时不能为空。一份配置只使用一个来源位置。"),
            _f("boundary.selection.sides", "list[str]", "活动边界边",
               options=["west", "east", "south", "north"],
               default=["west", "east", "south", "north"]),
            _f("boundary.interpolation.method", "enum", "空间映射",
               options=["nearest", "linear"], default="nearest",
               note="linear 为首版两点插值；投影落在线段外视为覆盖失败。"),
            _f("boundary.interpolation.max_distance_km", "float",
               "源站到目标点的最大球面距离（千米）",
               note="启用时必填且必须为正。50 只是示例，不是物理推荐值。"),
            _f("boundary.validation.max_time_gap_seconds", "int",
               "允许的最大时间缺口（秒）",
               note="启用时必填且必须为正。10800 只是示例。"),
            _f("boundary.resources.memory_limit_mb", "int",
               "ww3_bounc 转换内存预算（MiB）", default=1024),
            _f("ww3_grid.SPECTRUM%THOFF", "float", "第一个方向的相对偏移 [-0.5, 0.5]",
               default=0),
        ...
        "environment": [
            ...
            {"name": "WW3TOOL_PYTHON", "description": "local.sh / server.sh 调用边界 runtime 的解释器"},
