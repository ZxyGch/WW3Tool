    ...
    return [
        (
            ...
            [
                ...
                ("validate [workdir]", tr("icli_help_validate", "校验当前配置文件")),
                ("config [workdir]", tr("icli_help_config", "显示当前配置摘要")),
                ("print-params [workdir]", tr("icli_help_print_params", "输出当前 params.yml 内容")),
            ],
        ),
        (
            tr("icli_grp_preproc", "预处理"),
            [
                ("prepare-forcing [workdir]", tr("icli_help_prepare_forcing", "准备强迫场（Step 1）")),
                (
                ...
                ("generate-grid [workdir]", tr("icli_help_generate_grid", "生成网格（Step 2）")),
                ("prepare-ww3 [workdir]", tr("icli_help_prepare_ww3", "仅生成 WW3 namelist（不重跑强迫场和网格）")),
                ("recommend-cfl [workdir]", tr("icli_help_recommend_cfl", "按 CFL 公式推荐时间步长")),
                ("recommend-grid [workdir]", tr("icli_help_recommend_grid", "按区域范围推荐网格间距/分辨率（--coarse 偏粗 / --fine 偏细）")),
                ("run-workflow [workdir]", tr("icli_help_run_workflow", "完整预处理流程")),
                ("local-run [workdir]", tr("icli_help_local_run", "执行当前工作目录的 local.sh")),
            ],
        ),
        (
            tr("icli_grp_plot", "后处理 / 绘图"),
            [
                ("plot-wave-maps [workdir] [--contour]", tr("icli_help_plot_wave_maps", "生成波高填色图或等值线图")),
                ("plot-spectrum [workdir] [--mode ...] [--station N]", tr("icli_help_plot_spectrum", "生成方向谱图")),
                ("plot-jason3 [workdir]", tr("icli_help_match_jason3", "WW3 结果与 Jason-3 卫星数据匹配")),
                ("plot-jason3-swh [workdir]", tr("icli_help_jason3_swh", "绘制 Jason-3 卫星 SWH / 轨迹图")),
                ("download-jason3 [workdir]", tr("icli_help_download_jason3", "下载 Jason-3 L2 数据")),
                ("plot-ndbc [workdir]", tr("icli_help_match_ndbc", "WW3 结果与 NDBC 浮标匹配")),
                ("download-ndbc [workdir]", tr("icli_help_download_ndbc", "下载 NDBC 浮标观测数据")),
            ],
        ),
        (
            tr("icli_grp_remote", "远程运维"),
            [
                ("connect-test [workdir]", tr("icli_help_connect_test", "测试 SSH 连接")),
                ("ssh [workdir]", tr("icli_help_ssh", "打开交互式 SSH 终端")),
                ("slurm-idle [workdir]", tr("icli_help_slurm_idle", "查看 Slurm 空闲 CPU 资源")),
                (
                    "confirm-slurm [workdir] [full|half]",
                    tr(
                        "icli_help_confirm_slurm",
                        "按 params.yml 的 Slurm 配置写 server.sh；full/half 自动选取空闲 CPU",
                    ),
                ),
                ("list-files [workdir]", tr("icli_help_list_files", "列出远程工作目录文件")),
                ("upload [workdir] --confirm", tr("icli_help_upload", "上传本地工作目录到远程")),
                ("submit [workdir]", tr("icli_help_submit", "在远程执行提交脚本")),
                ("check-status [workdir]", tr("icli_help_check_status", "检查远程任务状态")),
                ("queue-status [workdir]", tr("icli_help_queue_status", "查看 SLURM 队列")),
                ("download-results [workdir] [--nested]", tr("icli_help_download_results", "下载远程 WW3 结果")),
                ("download-log [workdir]", tr("icli_help_download_log", "下载远程日志文件")),
                ("clear-remote [workdir] --confirm", tr("icli_help_clear_remote", "清空远程工作目录")),
                ("cancel-job [workdir] <job_id>", tr("icli_help_cancel_job", "取消 SLURM 任务")),
                ("ntfy-watch [workdir]", tr("icli_help_ntfy_watch", "注入常驻 ntfy 监听到远程服务器")),
                ("ntfy-watch-job [workdir] <job_id>", tr("icli_help_ntfy_watch_job", "为指定 SLURM 任务注入一次性 ntfy 监听")),
            ],
        ),
        (
            tr("icli_grp_aux", "辅助"),
