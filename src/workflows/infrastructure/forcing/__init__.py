...
本包负责 NetCDF 强迫场文件的 I/O、变量检测、路径命名、必要兼容性修复与风场归一化，
为应用层 ``forcing_preparation`` 等模块提供可复用的底层能力。

[EN] This package handles I/O, variable detection, path naming, necessary compatibility fixes, and wind
...
- ``file_service``：复制/移动文件，标准化坐标名与时间单位，修复旧风场变量名与纬度递减；
...
- ``file_service``: Copy/move files, standardize coordinate names and time units, fix legacy wind variable names and descending latitude;
