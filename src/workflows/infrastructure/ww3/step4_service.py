...
class StepFourServiceMixin:
    ...
    def _load_output_schemes_to_combo(self, preserve_selection=None):
        ...
        Args:
            preserve_selection: 如果提供，刷新后保持选择该方案；否则默认选择"Default"
        """
        ...
        # 优先从 PipelineConfig (params.yml) 读取输出方案
        ...
        if scheme_names:
            ...
            # 如果指定了要保留的选择且该方案存在，则选择它；否则默认选择"Default"
    ...
    def _on_output_scheme_changed(self, scheme_name):
        ...
        # 优先从 PipelineConfig (params.yml) 读取方案
        loaded_cfg = getattr(self, '_loaded_config', None)
        if loaded_cfg is not None and hasattr(loaded_cfg, 'presets'):
            schemes = dict(loaded_cfg.presets.output_scheme)
        else:
