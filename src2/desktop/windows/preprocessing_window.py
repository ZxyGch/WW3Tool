"""Main desktop shell backed by src2 workflows instead of legacy src window code."""
...
class PreprocessingWindow(FluentWindow, ImageGalleryHost):
    ...
    def _log_style(self) -> str:
        if self._is_dark_theme():
            return (
                "PlainTextEdit { background-color: #2d2d2d; border: 0.5px solid #404040;"
                " border-radius: 4px; padding-left: 2px; }"
                " PlainTextEdit:focus { border: 0.5px solid #404040; padding-left: 2px; }"
                " PlainTextEdit:hover { border: 0.5px solid #404040; padding-left: 2px; }"
            )
        return (
            "PlainTextEdit { background-color: transparent; border: 0.5px solid #D0D0D0;"
            " border-radius: 4px; padding-left: 2px; }"
            " PlainTextEdit:focus { border: 0.5px solid #D0D0D0; padding-left: 2px; }"
            " PlainTextEdit:hover { border: 0.5px solid #D0D0D0; padding-left: 2px; }"
        )
    ...
    def _load_params(self, path: Path, *, update_workdir_ui: bool = True) -> None:
        try:
            # 仅用于把 params.yml 覆盖到主页表单：用最宽松的 plot 阶段（不做强校验），
            # 这样只要 params.yml 能解析就完整回填，避免因网格后端/强迫场等校验失败而不刷新。
            # 严格校验留到运行/确认参数时。
        ...
        for key in ("wind", "current", "level", "ice"):
            ...
            if value is not None and not Path(value).exists():
                self._append_log(tr("forcing_file_missing_tip", "提示：forcing.{key} 文件不存在：{path}").format(key=key, path=value))
    ...
    @staticmethod
    def _effective_server_path(config: PipelineConfig) -> str:
        """显示用服务器路径：config.json SERVER_PATH 优先（与运行时覆盖一致），否则取 params。"""
        try:
            cfg_path = str(_load_runtime_config().get("SERVER_PATH", "") or "").strip()
        except Exception:
            cfg_path = ""
        return cfg_path or config.server.remote_dir

    def _show_home(self) -> None:
        """切回主页步骤页，并按当前 config.json 刷新第六步服务器路径与第四步可选分组显隐。"""
        self.left_stacked.setCurrentIndex(0)
        if hasattr(self, "_server_ops_panel"):
            fallback = self._loaded_config.server.remote_dir if self._loaded_config else ""
            try:
                cfg_path = str(_load_runtime_config().get("SERVER_PATH", "") or "").strip()
            except Exception:
                cfg_path = ""
            self._server_ops_panel.set_server_path(cfg_path or fallback)
        if hasattr(self, "_ww3_panel"):
            try:
    ...
    def _persist_server_remote_dir(self) -> bool:
        ...
        try:
            ...
            return False
        self._append_log(tr("server_path_saved_to_params", "已保存服务器路径到当前工作目录 params.yml：{path}").format(path=destination))
        return True
    ...
    def _persist_current_form_to_workdir_params(
        ...
        *,
        ...
        """主页流程统一入口：先把当前表单写入当前工作目录 params.yml。"""
        params_path = self._current_workdir_params_path(create=True)
        if params_path is None:
            return None
        try:
    ...
    def _auto_configure_timesteps(self) -> None:
        ...
        InfoBar.success(
            title=tr("step4_auto_timesteps_title", "时间步长已更新"),
            content=summary,
            duration=5000,
            parent=self,
        )
    ...
    def _plot_ndbc_station_map(self) -> None:
        ...
        if not ndbc_folder or not os.path.isdir(ndbc_folder):
            ndbc_folder = ensure_project_data_dir("NDBC_PATH", "ndbc")
    ...
    def _on_forcing_done(self, result: object) -> None:
        ...
        if isinstance(result, ForcingStepState) and result.error:
            self._show_error(result.error)
        elif isinstance(result, ForcingStepState):
            self._persist_prepared_forcing(result.files)
        elif isinstance(result, dict) and result.get("error"):
            self._show_error(str(result["error"]))

    def _persist_prepared_forcing(self, files: Step1Files) -> None:
        if not files.existing_items():
            return
        params_path = self._current_workdir_params_path(create=True)
        if params_path is None:
            return
        try:
            destination = self._pipeline_vm.save_prepared_forcing_to_params(params_path, files)
        except Exception as exc:
        ...
        self._params_label.setText(str(destination))
        self._append_log(
            tr("forcing_params_saved", "已将转换后的强迫场路径写入 params.yml：{path}").format(path=destination)
        )
    ...
    def _on_grid_done(self, result: object) -> None:
        ...
        if isinstance(result, PipelineStepState) and result.error:
            ...
            self._mark_grid_generated()

    def _mark_grid_generated(self) -> None:
        params_path = self._current_workdir_params_path(create=False)
        if params_path is None:
            return
        try:
            destination = self._pipeline_vm.save_grid_generated_to_params(params_path, True)
        except Exception as exc:
            self._show_error(tr("grid_params_save_failed", "保存网格状态到 params.yml 失败：{error}").format(error=exc))
            return
        self._params_path = destination
        self._params_label.setText(str(destination))
        self._append_log(tr("grid_params_saved", "已将网格生成状态写入 params.yml：{path}").format(path=destination))
