"""设置子界面：编辑 ``public/config.json`` 与非结构/SMC 网格 JSON。
...
class SettingsInterface(QWidget):
    ...
    def _build_nearshore_card(self) -> None:
        grid = self._card(tr("nearshore_config", "近岸配置"))
        self._text(grid, 0, 0, tr("set_grid_zlim", "海岸线限制深度 (米)："), "GRID_ZLIM")
        self._text(grid, 1, 0, tr("set_grid_dmin", "绝对最小水深 (米)："), "GRID_DMIN")
        self._reset_button(grid, 2, self._reset_nearshore_defaults)
    ...
    def _reset_nearshore_defaults(self) -> None:
        for key, value in {"GRID_ZLIM": "-0.1", "GRID_DMIN": "2.5"}.items():
            if key in self._fields:
                self._fields[key].setText(value)
        self._save_config_now()
    ...
    def _manage_cpu_group(self) -> None:
        ...
        self._config = self._vm.load()
        self._toast(tr("set_cpu_saved","已保存 {n} 个 CPU").format(n=len(cpu_group)))
    ...
    def _build_scheme_card(self) -> None:
        ...
        form.addWidget(self._scheme_name_edit, 0, 1)
        ...
        form.addWidget(self._label(tr("current_scheme", "当前方案："), word_wrap=False), 0, 2)
        form.addWidget(self._scheme_combo, 0, 3)
