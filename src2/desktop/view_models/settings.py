"""设置页视图模型：封装 ``public/config.json`` 与网格 JSON 的读写。
...
class SettingsViewModel:
    ...
    def load(self) -> dict[str, Any]:
        """返回与 ``DEFAULT_CONFIG`` 合并、方案规范化后的配置。"""
        return runtime_config.load_config()

    def save(self, updates: dict[str, Any]) -> bool:
        """将 ``updates`` 合并进现有配置并写回 ``config.json``。"""
        config = runtime_config.load_config()
        config.update(updates)
        return runtime_config.save_config(config)
    ...
    def default_st(self) -> str:
        return str(runtime_config.load_config().get("DEFAULT_ST", "") or "")
