...
def _resolve_remote_dir(config: PipelineConfig) -> str:
    ...
    if not remote_dir:
        raise ValueError(
            tr("remote_dir_missing", "server.remote_dir 未配置。请在 params.yml 的 server: 段填写远程工作目录。")
        )
...
def run_upload(
    ...
    **必须传入 ``confirmed=True``**；否则立即返回错误，防止误触发上传。
    ...
    try:
        ...
        client.connect(log=logger.log)
        client.upload_folder(local_dir, remote_dir, log=logger.log)
        return RemoteResult(success=True, messages=list(logger.messages))
    except Exception as exc:
...
def run_submit(
    ...
    *,
    ...
    try:
        remote_dir = _resolve_remote_dir(config)
        client.connect(log=logger.log)
        code = client.exec_script(script, remote_dir, log=logger.log)
        return RemoteResult(
            success=(code == 0),
...
def run_check_status(
    ...
    try:
        remote_dir = _resolve_remote_dir(config)
        client.connect(log=logger.log)
        status = client.check_completion(remote_dir, log=logger.log)
        msg_map = {
...
def run_queue_status(
    ...
    try:
        client.connect(log=logger.log)
        output = client.queue_status(log=logger.log)
        if output:
...
def run_list_files(
    ...
    try:
        ...
        output, err, code = client.exec_command(cmd, log=logger.log, timeout=10)
        if code != 0:
...
def run_download_results(
    ...
    *,
    ...
    try:
        ...
        files = client.download_files(search_dir, local_save, _is_ww3_nc, log=logger.log)
...
def run_download_log(
    ...
    try:
        ...
        files = client.download_files(remote_dir, local_dir, _is_log, log=logger.log)
        return RemoteResult(success=True, data=files, messages=list(logger.messages))
    except Exception as exc:
...
def run_clear_remote(
    ...
    **必须传入 ``confirmed=True``**；CLI 对应 ``--confirm``。
    ...
    try:
        remote_dir = _resolve_remote_dir(config)
        client.connect(log=logger.log)
        client.clear_remote_dir(remote_dir, log=logger.log)
        return RemoteResult(success=True, messages=list(logger.messages))
    except Exception as exc:
...
def run_cancel_job(
    ...
    try:
        client.connect(log=logger.log)
        client.cancel_job(job_id, log=logger.log)
        return RemoteResult(success=True, messages=list(logger.messages))
    except Exception as exc:
