...
def _build_retry_session():
    ...
    adapter = HTTPAdapter(
        max_retries=retry,
        pool_connections=_CATALOG_SCAN_WORKERS,
        pool_maxsize=_CATALOG_SCAN_WORKERS,
    )
...
def _download_jason3_worker(time_range, local_folder, base_catalog_url, log_queue, result_queue):
    """下载指定时间范围的 Jason-3 L2 NetCDF 产品到本地目录。
    ...
    base_catalog_url : str
        NCEI Jason-3 产品根 URL（如 ``https://www.ncei.noaa.gov/data/oceans/jason3/``）。
    log_queue : queue-like
        日志回传队列（兼容 ``multiprocessing.Queue`` / ``ImmediateQueue``）。
    result_queue : queue-like
        结果回传队列；发送 ``dict`` 含 ``ok / downloaded / skipped / failed / total``。
