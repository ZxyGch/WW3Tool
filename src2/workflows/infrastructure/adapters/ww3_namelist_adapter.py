...
def _resolve_st_name(config: PipelineConfig, app_config: Dict[str, Any]) -> str:
    ...
    if versions and isinstance(versions, list) and isinstance(versions[0], dict):
        return str(versions[0].get("name") or "ST2")
...
def _merged_runtime_config(config: PipelineConfig) -> Dict[str, Any]:
    """合并磁盘 ``config.json``、默认项与当前 ``PipelineConfig`` 中的 WW3/SLURM 参数。"""
    try:
        merged = copy.deepcopy(runtime_config.load_config())
    except Exception:
        merged = copy.deepcopy(runtime_config.DEFAULT_CONFIG)
    ...
    if config.slurm.server_script_path is not None:
        merged["SERVER_SCRIPT_PATH"] = str(config.slurm.server_script_path)
    ...
    merged.update(
        {
            ...
            "GRID_ZLIM": parameters["GRID%ZLIM"],
            "GRID_DMIN": parameters["GRID%DMIN"],
        }
    )
    ...
    schemes = copy.deepcopy(merged.get("OUTPUT_VARS_SCHEMES") or {})
...
def _apply_ww3_grid_settings(config: PipelineConfig, logger: CoreLogger) -> None:
    ...
    values = {
        ...
        "GRID_NML": {
            "GRID%ZLIM": f"  GRID%ZLIM         =  {parameters['GRID%ZLIM']}\n",
            "GRID%DMIN": f"  GRID%DMIN         =  {parameters['GRID%DMIN']}\n",
        },
    }
    for path in _ww3_grid_paths(config):
        if not path.is_file():
            logger.log(f"未找到 ww3_grid.nml，跳过数值参数写入：{path}")
            continue
        ...
        if changed:
            ...
            logger.log(f"已应用 ww3_grid 数值参数：{path}")
