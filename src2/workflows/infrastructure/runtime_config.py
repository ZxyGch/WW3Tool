...
本模块管理 ``public/config.json`` 的加载/保存、默认配置合并，以及桌面端与 CLI
共用的路径解析（项目根、网格生成器、默认工作目录、Step 1 强迫场目录等）。

模块导入时会调用 ``reload_config()``，将常用项同步到模块级全局变量（如
``MATLAB_PATH``、``SERVER_HOST``），供遗留代码直接引用；新代码优先调用
``load_config()`` 获取完整字典。
"""
import copy
import json
...
DEFAULT_CONFIG = {
    ...
    # 文件分割方式（"none"、"hour"、"day"、"month"、"year"）
    "FILE_SPLIT": "year",
...
def load_config():
    """从 ``public/config.json`` 加载配置并与 ``DEFAULT_CONFIG`` 合并。

    若文件不存在则写入默认配置后返回；读取失败时打印警告并回退到默认项。
    合并后会规范化输出变量方案名称（``normalize_output_vars_schemes``）。
    """
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                config = json.load(f)
            original_run_mode = config.get("RUN_MODE") if isinstance(config, dict) else None
            # 合并默认配置，确保所有键都存在
            merged_config = DEFAULT_CONFIG.copy()
            merged_config.update(config)
            normalize_output_vars_schemes(merged_config)
            merged_config["RUN_MODE"] = normalize_run_mode(merged_config.get("RUN_MODE"))
            if original_run_mode != merged_config["RUN_MODE"]:
            ...
            fallback_config["RUN_MODE"] = normalize_run_mode(fallback_config.get("RUN_MODE"))
...
def reload_config():
    """重新加载配置并更新本模块全局变量（MATLAB_PATH、SLURM、服务器等）。

    在设置页保存配置或外部修改 ``config.json`` 后应调用，使模块级常量与磁盘一致。
    """
