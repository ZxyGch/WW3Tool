...
本目录提供 **WAVEWATCH III 结构化网格**（矩形 / 曲线网格：水深、陆海掩膜、障碍物等）的生成能力。

- **原始代码来源：** [Ifremer gridgen](https://gitlab.ifremer.fr/wave/tools/gridgen)（GNU GPL v3.0 或更新版本）。
- **`pygridgen/` 目录：** 其中的 Python 代码为 **通过 AI 从原始工程转换** 得到的实现；**`gridgen/`** 目录保留对应的 MATLAB 等原始风格实现，二者均面向 WAVEWATCH III 兼容输出。
...
本工具是面向 **WAVEWATCH III（WW3）波浪模式** 的 **结构化网格生成器**：在矩形或曲线网格上准备模式所需的网格文件（水深、掩膜、障碍物等），供 WW3 预处理与运行使用。MATLAB 工作流见 **`gridgen/`**，Python 工作流见 **`pygridgen/`**（见文首溯源说明）。
...
脚本会从 **GitHub** 上 WW3Tool 的 `data` 资源下载 `part_aa`～`part_ad`，合并为 `reference_data.zip` 并解压到 `reference_data/`。  
若需改为从各上游站点分别拉取（较慢），可使用：`python3 get_reference_data.py --legacy`。

**可选：手动下载**

若 GitHub 下载慢或不可用，可下载完整的 **`reference_data`** 压缩包（或等价内容），解压后使 `WW3-Grid-Generator/reference_data/` 具备下文「参考数据」所列目录结构：
...
- **百度网盘：** <https://pan.baidu.com/s/1SxQEfiaomdi3CXFOXC6DMw?pwd=cb48>（提取码：`cb48`）

若目录不存在请先创建：`mkdir -p WW3-Grid-Generator/reference_data`。
...
**注意**：
- 确保已正确设置 Python 路径，指向 `WW3-Grid-Generator/structured_generator/pygridgen` 目录
- 默认参考数据目录为 `WW3-Grid-Generator/reference_data/`，输出目录为 `WW3-Grid-Generator/result/`
...
4. **`grid.nml`**（`write_ww3meta` 输出，与 MATLAB/Python `gridgen` 一致）
   - 格式：ASCII 文本文件
...
**解决方案**：
- 在 `WW3-Grid-Generator/` 下运行 `python3 get_reference_data.py`，或按「安装」一节从 Ydray / OneDrive / 百度网盘手动获取并解压到 `reference_data/`
...
**解决方案**：
- 在 `WW3-Grid-Generator/` 下运行 `python3 get_reference_data.py`，或手动恢复 `reference_data` 中的 `gebco.nc`（及可选的 `etopo1.nc`、`etopo2.nc`）
- 检查 `reference_data` 目录是否包含相应的 `.nc` 文件：
  - `gebco.nc`（捆绑包中常用，推荐）
  - `etopo1.nc`（可选）
  - `etopo2.nc`（可选）
- 检查 `ref_grid` 参数是否与已有文件一致（`gebco`, `etopo1`, `etopo2`）
...
- **默认：** 在 `WW3-Grid-Generator/` 下执行 `python3 get_reference_data.py`，从 **GitHub** `data` Release 分卷下载并解压。
- **手动：** 若需网盘镜像，见「安装 → 1. 下载参考数据」中的 Ydray、OneDrive、百度网盘链接；解压后目录结构须与下文一致。

### 内容说明（典型）

- **水深：** 捆绑包中一般为 **`gebco.nc`**；亦可自行放入 **`etopo1.nc`**、**`etopo2.nc`** 等以配合 `ref_grid`
- **岸线 / 掩膜相关：** 多分辨率 **`coastal_bound_*.mat`**，以及 **`optional_coastal_polygons.mat`**、**`user_polygons.flag`** 等
