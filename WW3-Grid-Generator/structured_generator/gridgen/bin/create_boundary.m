...
[lon,lat] = read_ww3meta([in_dir,'/ww3_grid.nml.',fname]);  % read meta file
...
fnameb(fnameb=='''') = [];
[lonb,latb] = read_ww3meta([in_dir,'/ww3_grid.nml.',fnameb]);
[Nyb,Nxb] = size(lonb);
...
filename = [in_dir,'/ww3_grid.nml.',fname]
...
if (strcmp(type,'rect'))
    ...
    axesm eckert4
    geoshow(lat,lon,m,'DisplayType','surface')
    framem
    gridm
end
shading interp;
...
if (strcmp(type,'rect'))
    ...
    axesm eckert4
    geoshow(latb,lonb,mb,'DisplayType','surface')
    framem
    gridm
end
shading interp;
...
if (strcmp(type,'rect'))
    ...
    axesm eckert4
    geoshow(lat,lon,m_new,'DisplayType','surface')
    framem
    gridm
end
shading interp;
...
if (strcmp(type,'curv'))
    disp('no bound ascii file for curv')
    return
end
