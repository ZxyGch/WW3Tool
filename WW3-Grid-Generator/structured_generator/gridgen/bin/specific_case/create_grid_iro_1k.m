...
% 0.b Design grid parameters
  ...
  ref_grid = 'xyz_iro_1k';                  % reference grid source 
                                     %      etopo1 = Etopo1 grid
                                     %      etopo2 = Etopo2 grid
                                     %         xyz = ASCII .log grid
  icoords= 0;
  boundary = 'inter';                 % Option to determine which GSHHS 
                                     %  .mat file to load
...
%    [lon,lat,depth] = load_grd(ref_dir,ref_grid,grid_box,dx,dy,-10.,MSL_LEV,DRY_VAL);

  depth = generate_grid_xyz(lon,lat,ref_dir,ref_grid,MSL_LEV,CUT_OFF,DRY_VAL);
                                                            % this grid has been generated using a 
...
%   [b,N1] = compute_boundary_iro(coord,bound,icoords);
  [b,N1] = compute_boundary(coord,bound);

  if (opt_poly == 1)
%       [b_opt,N2] = compute_boundary_iro(coord,bound_user,icoords);     
...
% 5. Output to ascii files for WAVEWATCH III
  ...
  %% WARNING +4 IS HERE FOR CHART DATUM TO MSL CORRECTION ...
  %% MUST BE IMPROVED WITH PROPER CORRECTION ... 
  
  d = round((depth+4.)*depth_scale);
