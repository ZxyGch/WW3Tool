...
%|                                         (25 km; 1866 polygons)         |
%|   min_dist : threshold defining the minimum distance between the edge  |
%|             of polygon and the inside/outside boundary.                |
            ...
            if (~inside_grid)
                ...
                if (isempty(loc1))
                    if (domain_inb_lth == length(px))
                        ...
                        %figure(555);
                        %plot(bound(i).x,bound(i).y,'y-'); hold on;
                        %plot(px,py,'g-');
                    end
                    ...
                    for j = 1:in2out_count
                        ...
                        if (on_points(out2in(j)+1) == 1)
                            ...
                            for k = 1:length(px)-1
                                ...
                                if (abs(1-(d1+d2)/d) < 0.001) % TEST < eps
                                    out2in_gridbox(j) = k;
                                    break;
                                end
                                % else
                    ...
                    if (in2out_count > 0)
                        ...
                        while(~isempty(find(subseg_acc == 0,1)))
                            ...
                            if (subseg_acc(1) == 0)
                                ...
                                min_pos = NaN;
                                min_dist = max(out2in_gridboxdist)+eps;
                                for j = 1:in2out_count
                                    if (subseg_acc(j) == 0)
                                        if (out2in_gridboxdist(j) < min_dist)
                                            min_dist = out2in_gridboxdist(j);
                                            min_pos = j;
                                        end
                                    end
                                end
                            ...
                            if (j <= 0)
                                error('Error: no intersection point found.');
                            end
                            ...
                            while (close_bound == 0)
                                ...
                                if (isempty(find(subseg_acc == 0,1)))
                                    ...
                                    min_pos = 0;
                                    relative_distdiff = abs(out2in_gridboxdist-start_dist);
                                    min_dist = max(relative_distdiff)+eps; %20+eps;
                               
                                    %@@@ Check all segments
                                    for k1 = curr_seg+1:in2out_count
                                    %for k1 = 1:in2out_count
                                        if (relative_distdiff(k1) > eps ...
                                                && relative_distdiff(k1) < min_dist)
                                            min_pos = k1;
                                            min_dist = relative_distdiff(k1);
                                        end
                                    end
                                    
                                    if min_pos == 0 && curr_seg == in2out_count
                                        min_pos = curr_seg;
                                    end

                                    %figure;plot(bound_x,bound_y);
                                    if (min_pos == 0)
                                        for k1 = 1:out2in_count
                                            if (out2in_gridboxdist(k1) < min_dist)
                                                min_pos = k1;
                                                min_dist = out2in_gridboxdist(k1);
                                            end
                                        end
                                    end
                                    
                                    if (min_pos <=0)
                                        fprintf(1,'[ERROR] min_pos : %d\n',min_pos);
                                        pause;
                                    end
                            ...
                            % add additional points to smooth truncated
                            % polygon if needed
