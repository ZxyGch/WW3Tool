function depth_sub = generate_grid(type,x,y,ref_dir,bathy_source,limit,cut_off,dry,avg,varargin)
...
%|  avg          : 2D Averaging method ('max' or 'mean')               |
...
if narg == 12 %@@@ Extra 3 arguments define the lat, lon and depth var names
    var_x = varargin{1};
    var_y = varargin{2};
    var_z = varargin{3};
...
fname_base = [ref_dir,'/',bathy_input,'.nc'];
...
if (strcmp(type,'lamb'))
    ...
    for ilat=1:Ny
        for ilon=1:Nx
            depth_sub(ilat,ilon) = netcdf.getVar(f,varid_dep,...
                [ilon-1 ilat-1],[1 1],[1 1],'single')';
...
elseif (strcmp(type,'rect') || strcmp(type,'curv'))
    
    varid_lon = netcdf.inqVarID(f,var_x);
    varid_lat = netcdf.inqVarID(f,var_y);
    varid_dep = netcdf.inqVarID(f,var_z);
    
    dimid_lon = netcdf.inqDimID(f,var_x);
    dimid_lat = netcdf.inqDimID(f,var_y);
    
    [~,Nx_base]=netcdf.inqDim(f,dimid_lon);
    [~,Ny_base]=netcdf.inqDim(f,dimid_lat);
    
    lat_coord = double(netcdf.getVar(f,varid_lat));
    lat_coord = lat_coord(:);
    lon_coord = double(netcdf.getVar(f,varid_lon));
    lon_coord = lon_coord(:);
    if numel(lat_coord) ~= Ny_base || numel(lon_coord) ~= Nx_base
        netcdf.close(f);
        error('generate_grid:CoordinateSize', ...
            'Lat/lon 1D lengths (%d,%d) do not match dims (%d,%d).', ...
            numel(lat_coord), numel(lon_coord), Ny_base, Nx_base);
    end
    % Omit actual_range (optional); derive span from coordinate vectors (GEBCO-style files).
    lats_base = min(lat_coord);
    late_base = max(lat_coord);
    dy_base = (late_base - lats_base) / max(Ny_base - 1, 1);
    lons_base = min(lon_coord);
    lone_base = max(lon_coord);
    dx_base = (lone_base - lons_base) / max(Nx_base - 1, 1);
    ...
    lat_base = single(netcdf.getVar(f,varid_lat,lat_start-1,count_lat,1));
    
    if (lon_end <= lon_start)
        count_lon1 = (Nx_base - lon_start) + 1;
        count_lon2 = (lon_end - 2) + 1;
        lon1 = single(netcdf.getVar(f,varid_lon,lon_start-1,count_lon1,1));
        lon2 = single(netcdf.getVar(f,varid_lon,1,count_lon2,1));
        lon_base = [lon1;lon2];
        dep1 = single(netcdf.getVar(f,varid_dep,...
            [lon_start-1 lat_start-1],[count_lon1 count_lat],[1 1]))';
        dep2 = single(netcdf.getVar(f,varid_dep,...
            [1 lat_start-1],[count_lon2 count_lat],[1 1]))';
        depth_base = [dep1 dep2];
    else
        count_lon = (lon_end - lon_start) + 1;
        lon_base = single(netcdf.getVar(f,varid_lon,lon_start-1,count_lon,1));
        depth_base = single(netcdf.getVar(f,varid_dep,...
            [lon_start-1 lat_start-1],[count_lon count_lat], [1 1]))';
    end
    netcdf.close(f);
    ...
    clear lon_base_tmp depth_base_tmp;
    ...
    for j = 1:Nx
        for k = 1:Ny
            ...
            if (ndx <= 1 && ndy <= 1)
                ...
                den = dx_base*dy_base;
                [~,lon_prev] = min(abs(lon_base-x(k,j))); % index of the nearest data point from x(k,j)
                ...
                if (~isempty(loc))
                    ...
                    if (Ntt/Nt > limit)
                        if strcmp(avg,'p75')
                          depth_sub(k,j) = feval('prctile',depth_tmp_incell(loc),75);
                        else
                          depth_sub(k,j) = feval(avg,depth_tmp_incell(loc));
                        end
                    else
                        depth_sub(k,j) = dry;
                    end
        ...
        end    %@@@ end of for loop through all the rows (latitudes)
        
    end %@@@ end of for loop through all the columns (longitudes)
    
end %@@@ end of section on rectilinear grid
