...
也可不发布直接本地装：`brew install ./public/packaging/Formula/ww3tool.rb`
...
```bash
./public/packaging/install.sh   # 把仓库内 public/packaging/ww3tool 软链到 PATH（可 --prefix 指定目录）
```
...
```bash
./public/packaging/mcp/setup.sh        # 创建 mcp/.venv（Python 3.12）并安装 mcp SDK
```

### 2. 通用 stdio 配置

任意支持 MCP 的客户端，注册一个 stdio server，`command` 指向
`launch.sh` 即可（内容见 `public/packaging/mcp/config.example.json`）：

```json
{
  "mcpServers": {
    "ww3tool": {
      "command": "/绝对路径/WW3Tool/public/packaging/mcp/launch.sh",
      "args": []
    }
  }
}
...
```bash
public/packaging/mcp/.venv/bin/python public/packaging/mcp/tests/smoke_test.py
```
