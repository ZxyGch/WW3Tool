...
- `server.sh` / `local.sh` 在常规网格路径上无条件调用边界运行时，并先检查能否 import。`server.sh` 由本地工具生成后随工作目录上传，
  ...
  SHOU 于 2026-09-08 已升级到当时最新的发布版 0.1.30，但边界模块截至 2026-09-15 仍是本地未提交代码，从未进入发布版，
  服务器实测 `ModuleNotFoundError`；本次验收在提交时设 `WW3TOOL_ROOT` 指向上传的源码。发布含边界功能的版本后服务器需再升级一次，
  桌面端与服务器端版本不一致时仍会复现。
