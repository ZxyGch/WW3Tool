...
- `server.sh` / `local.sh` 在常规网格路径上无条件调用边界运行时，并先检查能否 import。`server.sh` 由本地工具生成后随工作目录上传，
  所以只要本地工具含边界支持、而服务器端安装不含边界模块，**所有常规网格作业都会失败，即使没开边界**。
  SHOU 于 2026-09-08 升级到当时最新的 0.1.30，那时边界模块尚未发布，服务器实测 `ModuleNotFoundError`；本次验收在提交时设
  `WW3TOOL_ROOT` 指向上传的源码。边界功能于 2026-09-15 随 0.2.0 发布，服务器端需升级到 0.2.0 及以上；
  桌面端与服务器端版本不一致时仍会复现，README 已写明。
