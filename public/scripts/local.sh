...
# 外部边界谱：用 WW3Tool 解释器调用公共 runtime。禁止静默改用 PATH 里别的 ww3_bounc。
...
run_boundary_runtime() {
    ...
    py="$(boundary_python)" || {
        echo "BOUNDARY_EXECUTABLE_MISSING: 找不到 Python，请设置 WW3TOOL_PYTHON"
        fail_exit 1
    }
    if ! "$py" -c "import workflows.infrastructure.boundary.runtime" >/dev/null 2>&1; then
        echo "BOUNDARY: 无法 import workflows.infrastructure.boundary.runtime；请设置 WW3TOOL_PYTHON 为 WW3Tool 解释器"
        fail_exit 1
    fi
    run_step "boundary ($phase)" "$py" -m workflows.infrastructure.boundary.runtime --workdir "$SCRIPT_ROOT" --phase "$phase" --lock-owner "$BOUNDARY_LOCK_OWNER"
}
...
if [ "$GRID_TYPE" = "nested" ]; then
    if [ "$(params_boundary_mode | tr '[:upper:]' '[:lower:]')" = "external_spectra" ]; then
        echo "BOUNDARY_GRID_UNSUPPORTED: 首版不支持嵌套多网格外部谱"
        fail_exit 1
    fi
